...
RESUME_PIN = int(os.environ.get("RESUME_PIN", "33"))  # BOARD pin 33 — Resume input
RESUME_POLL_INTERVAL = 0.1  # seconds between GPIO input polls
...
if RELAY_ENABLED:
    try:
        ...
        # Pin 15: Resume input (pull-down, idle LOW, detect HIGH)
        GPIO.setup(RESUME_PIN, GPIO.IN, pull_up_down=GPIO.PUD_DOWN)
        print(f"\u2713 Relay GPIO initialized")
        print(f"  Pin {RELAY_PIN}: Relay OUTPUT (HIGH=OFF, LOW=ON)")
        print(f"  Pin {RESUME_PIN}: Resume INPUT (pull-down, idle LOW, detect HIGH)")
        print(f"  Flow: Defect → Pin {RELAY_PIN} LOW (stop) → wait Pin {RESUME_PIN} HIGH (resume) → Pin {RELAY_PIN} HIGH (run)")
    except ImportError:
...
# Relay status for UI
relay_status = "IDLE"  # IDLE, RELAY_ON, WAITING_RESUME, RESUMING
...
def activate_relay():
    """Turn relay ON (Pin 7 → LOW) — stops the machine"""
    global machine_paused
    ts = datetime.now().strftime("%H:%M:%S.%f")[:-3]
    if RELAY_ENABLED and GPIO:
        GPIO.output(RELAY_PIN, GPIO.LOW)  # LOW = relay ON
        print(f"  [{ts}] Pin {RELAY_PIN} → LOW (Relay ON, Machine STOPPED)")
    with pause_lock:
        machine_paused = True
    set_relay_status("RELAY_ON")
    print(f"  [{ts}] \u26a0 RELAY ACTIVATED — Machine STOPPED")

def wait_for_resume_signal():
    """Block until Pin 15 receives a sustained HIGH signal (resume)"""
    global machine_paused
    ...
    if RELAY_ENABLED and GPIO:
        initial = 'HIGH' if GPIO.input(RESUME_PIN) else 'LOW'
        print(f"  [{ts}] Waiting for resume signal on Pin {RESUME_PIN} (sustained HIGH)...")
        print(f"  [{ts}] Pin {RESUME_PIN} current state: {initial}")
        if initial == 'HIGH':
            print(f"  [{ts}] WARNING: Pin {RESUME_PIN} is already HIGH — waiting for it to go LOW first...")
            while GPIO.input(RESUME_PIN) == GPIO.HIGH:
                time.sleep(RESUME_POLL_INTERVAL)
            ts2 = datetime.now().strftime("%H:%M:%S.%f")[:-3]
            print(f"  [{ts2}] Pin {RESUME_PIN} is now LOW — ready to detect resume signal")
            time.sleep(0.5)  # Settle time

        while True:
            pin_state = GPIO.input(RESUME_PIN)
            if pin_state == GPIO.HIGH:
                # Debounce: require DEBOUNCE_COUNT consecutive HIGH reads
                high_count = 0
                for i in range(DEBOUNCE_COUNT):
                    time.sleep(DEBOUNCE_DELAY)
                    if GPIO.input(RESUME_PIN) == GPIO.HIGH:
                        high_count += 1
                    else:
                        break
                if high_count == DEBOUNCE_COUNT:
                    ts2 = datetime.now().strftime("%H:%M:%S.%f")[:-3]
                    print(f"  [{ts2}] Pin {RESUME_PIN} confirmed HIGH ({DEBOUNCE_COUNT} reads) — resume signal detected!")
                    break
                else:
                    ts2 = datetime.now().strftime("%H:%M:%S.%f")[:-3]
                    print(f"  [{ts2}] Pin {RESUME_PIN} false trigger (only {high_count}/{DEBOUNCE_COUNT} HIGH reads) — ignoring")
            time.sleep(RESUME_POLL_INTERVAL)
    else:
        # No GPIO — just wait forever (no web controls)
        print(f"  [{ts}] No GPIO — system halted, restart to resume")
        while True:
            time.sleep(1)

def deactivate_relay():
    """Turn relay OFF (Pin 7 → HIGH) — resumes the machine"""
    global machine_paused
    set_relay_status("RESUMING")
    ts = datetime.now().strftime("%H:%M:%S.%f")[:-3]
    if RELAY_ENABLED and GPIO:
        # Turn relay OFF
        GPIO.output(RELAY_PIN, GPIO.HIGH)  # HIGH = relay OFF
        print(f"  [{ts}] Pin {RELAY_PIN} → HIGH (Relay OFF, Machine RESUMED)")
    with pause_lock:
        machine_paused = False
    set_relay_status("IDLE")
    print(f"  [{ts}] \u2713 RELAY DEACTIVATED — Machine RESUMED")
...
print(f"  Resume Pin:    {'Pin ' + str(RESUME_PIN) if RELAY_ENABLED else 'N/A'}")
    ...
    <div class="container">
        ...
        <div class="stats-panel">
            ...
            <div class="stat-card" id="relayCard" style="border-color:#ffaa00;">
                ...
                <div class="value" id="relayStatus" style="font-size:18px;color:#ffaa00;">IDLE</div>
                <div style="margin-top:8px;font-size:12px;color:#666;">
                    <span>Relay: Pin <span id="relayPin">7</span></span> &nbsp;|&nbsp;
                    <span>Resume: Pin <span id="resumePin">15</span></span>
                </div>
            </div>
            <!-- Stats Cards -->
        ...
        function updateUI() {
            fetch('/api/stats').then(r=>r.json()).then(data => {
                ...
                if (data.relay_status) {
                    rs.textContent = data.relay_status;
                    if (data.relay_status === 'RELAY_ON') {
                        rs.style.color = '#ff4444';
                        rc.style.borderColor = '#ff4444';
                    } else if (data.relay_status === 'WAITING_RESUME') {
                        rs.style.color = '#ffaa00';
                        rc.style.borderColor = '#ffaa00';
                        rs.textContent = 'WAITING FOR RESUME (Pin ' + data.resume_pin + ')';
                    } else if (data.relay_status === 'RESUMING') {
                        rs.style.color = '#00d4ff';
                        rc.style.borderColor = '#00d4ff';
                    } else {
                ...
                if (data.resume_pin) document.getElementById('resumePin').textContent = data.resume_pin;
            }).catch(()=>{});
        }
...
try:
    while True:
        ...
        if defect_count > 0:
            ...
            print(f"  Total defect frames saved: {defect_frame_count}")

            # === RELAY: Stop machine on defect ===
            if not machine_paused:
                print(f"  >> ACTIVATING RELAY — Stopping machine!")
                activate_relay()

        # If machine is paused, wait for HIGH signal on Pin 33, then resume detection
        if machine_paused:
            ts_pause = datetime.now().strftime("%H:%M:%S.%f")[:-3]
            ...
            cv2.putText(paused_frame, f"Waiting for HIGH signal on Pin {RESUME_PIN}",
                        (paused_frame.shape[1]//2 - 300, paused_frame.shape[0]//2 + 30),
            ...
            # Block detection loop until HIGH signal on Pin 33
            print(f"  [{ts_pause}] Detection STOPPED — Waiting for HIGH signal on Pin {RESUME_PIN}...")
            wait_for_resume_signal()
            deactivate_relay()
            ts_resume = datetime.now().strftime("%H:%M:%S.%f")[:-3]
            print(f"  [{ts_resume}] Relay OFF — Detection RESUMED!")
            continue
        ...
        if ENABLE_WEB:
            ...
            with stats_lock:
                ...
                live_stats["resume_pin"] = RESUME_PIN
...
if RELAY_ENABLED and GPIO:
    try:
        ...
        pass
    GPIO.cleanup()
    print("\u2713 GPIO cleaned up (Pin 7 HIGH, relay OFF)")
if HAS_DISPLAY:
